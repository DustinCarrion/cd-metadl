...
class SimpleSequentialModel(SequentialModel):
    def __init__(self, models: list, last_layer: Module = Identity(), 
...
class ResNetWrap(SimpleSequentialModel):
    def __init__(self, model: ResNet):
        layer0 = nn.Sequential(model.conv1, model.bn1, model.act1, 
            model.maxpool)
        super().__init__([layer0, model.layer1, model.layer2, model.layer3, 
            model.layer4])
...
def rn_timm_mix(pretrained=True, name='swsl_resnet50', momentum=0.1):
    ...
    print('model: rn_timm_mix, name:', name, 'layer num:', model.num_layers, 
        'momentum:', momentum)
